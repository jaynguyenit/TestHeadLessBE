...
export function create(useQuery) {
    return (args) => {
        ...
        const { query } = router;
        if (hasPostId(query)) {
            params = Object.assign({ id: query.postId, idType: PostIdType.ID }, params);
        }
        else if (hasPostSlug(query)) {
            params = Object.assign({ id: query.postSlug, idType: PostIdType.SLUG }, params);
        }
        else if (hasPostUri(query)) {
            params = Object.assign({ id: query.postUri.join('/'), idType: PostIdType.URI }, params);
        }
