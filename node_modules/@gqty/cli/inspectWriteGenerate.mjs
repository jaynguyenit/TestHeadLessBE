...
import { getRemoteSchema } from './introspection.mjs';
import { writeGenerate } from './writeGenerate.mjs';
...
Please modify "${filepath.endsWith("package.json") ? "gqty" : "config"}.introspection.endpoint" in: "${filepath}".`);
  ...
  if (endpoint.startsWith("http://") || endpoint.startsWith("https://")) {
    ...
    if (files.length) {
      ...
      const gqlFiles = files.filter((file) => gqlextensions.includes(extname(file)));
      if (files.length > jsonFiles.length + gqlFiles.length) {
        throw Error(`Received invalid files. Type generation can only use .gql, .graphql or .json files`);
      }
      if (jsonFiles.length && gqlFiles.length) {
        throw Error(`Received mixed file inputs. Can not combine JSON and GQL files`);
      }
      if (jsonFiles.length && jsonFiles.length > 1) {
        throw Error(`Received multiple JSON introspection files, shoud only be one`);
      }
      const fileContents = await Promise.all(files.map((file) => promises.readFile(file, {
        encoding: "utf-8"
      })));
      if (extname(files[0]) === ".json") {
        const parsedFile = JSON.parse(fileContents[0]);
        let dataField;
  ...
  const generatedPath = await writeGenerate(schema, destination, genOptions, async (existingFile) => {
    var _a, _b;
