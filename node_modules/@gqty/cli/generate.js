...
async function generate(schema, {
  ...
  enumsAsConst != null ? enumsAsConst : enumsAsConst = (_c = gqtyConfig.enumsAsConst) != null ? _c : config.defaultConfig.enumsAsConst;
  scalarTypes || (scalarTypes = gqtyConfig.scalarTypes || config.defaultConfig.scalarTypes);
  endpoint || (endpoint = (_e = (_d = gqtyConfig.introspection) == null ? void 0 : _d.endpoint) != null ? _e : config.defaultConfig.introspection.endpoint);
  if (endpoint == null || !(endpoint.startsWith("http://") && endpoint.startsWith("https://"))) {
    endpoint = "/api/graphql";
  }
  react != null ? react : react = (_f = gqtyConfig.react) != null ? _f : config.defaultConfig.react;
  preImport != null ? preImport : preImport = (_g = gqtyConfig.preImport) != null ? _g : config.defaultConfig.preImport;
  subscriptions != null ? subscriptions : subscriptions = (_h = gqtyConfig.subscriptions) != null ? _h : config.defaultConfig.subscriptions;
    ...
    acum += `
      ...
      if (fieldValue.__args) {
        ...
        const argTypes = argsEntries.reduce((acum3, [argKey, argValue]) => {
          const argValueProps = gqty.parseSchemaType(argValue, argDescriptions[argKey]);
